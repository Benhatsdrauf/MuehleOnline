...
function isMovePossible(oldPos, newPos)
{
    return possibleMoves[oldPos].includes(newPos);
}

function isPositionOccupied(newPos, allOccupiedPositions) // allCurrentPositions = array
{
 ...
 * @returns {Boolean}
 */
function CheckForMill(playerOccupiedPositions) // CurrentPositions = array von einer Farbe
{
    return allPossibleMills.some(mills => mills.every(stone => playerOccupiedPositions.includes(stone)));
}

function Move(newPos, oldPos, playerOccupiedPositions)
{
